...
class SonicStick():
    ...
    def safetyCheck(self):
        ...
        if self.lastpos==None:
            ...
            if(self.lastpos<0):
                print('bad last position, do nothing')
            else:                
            ...
            try:
                ...
                if ((len(data) > 18) and (data[0:8] == "Art-Net\x00")):
                    ...
                    if ((opcode == 0x5000) and (protocolVersion >= 14)):
                        ...
                        idx = 18
                        if universe == 0 and ( rawbytes[idx+self.ID-1] != lastpos ):#or rawbytes[idx+self.ID] != lastspeed or rawbytes[idx+self.ID+1] != lastacc ):
    ...
    def motor_command_routine(self):
        while True:
            ...
            if(len(self.prQueue.move_list)>1):
                holdingRegisters = self.modbusClient.ReadHoldingRegisters(0x050E, 1,1)
                moveData=self.prQueue.move_list[0]
                print('current Pr:'+str(holdingRegisters[0])+' append Pr:'+str(moveData[4]))
...
if __name__ == '__main__':
    ...
    if args.T=='W': # Write current position to DB
        ...
        bigStick.home()
        time.sleep(5) 
        print('pos='+str(bigStick.read_current_pos()))
        time.sleep(5) 
